...
oneHalfKey='\x49\xbe'+key[:len(key)/2]+'\x66\x49\x0f\x6e'+movq14[3]
twoHalfKey='\x49\xbf'+key[len(key)/2:][len(key)/4:]+key[len(key)/2:][:len(key)/4]+'\x66\x49\x0f\x6e'+movq15[0]
# another 2^128 possibilities per block
insertKey='\x0f\xc6'+xmm[3][3]+'\x1b\x0f\xc6'+xmm[3][0]+'\x1b\x0f\x57'+xmm[0][0]+'\x0f\x57'+xmm[0][3]+'\x0f\x28'+xmm[5][0]
